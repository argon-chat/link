<template>
    <svg ref="svgRef" width="100%" height="100%" viewBox="0 0 700 200" xmlns="http://www.w3.org/2000/svg"
        ...
        <defs>
            <linearGradient id="textGradient" gradientUnits="userSpaceOnUse" cx="50%" cy="50%" r="25%">
                <stop v-if="hovered" offset="0%" stop-color="var(--yellow-100)" />
                <stop v-if="hovered" offset="25%" stop-color="var(--red-100)" />
                <stop v-if="hovered" offset="50%" stop-color="var(--blue-100)" />
                <stop v-if="hovered" offset="75%" stop-color="var(--cyan-100)" />
                <stop v-if="hovered" offset="100%" stop-color="var(--violet-100)" />
            </linearGradient>

            <!-- Radial Gradient -->
            <radialGradient id="revealMask" gradientUnits="userSpaceOnUse" r="20%" :cx="maskPosition.cx"
                :cy="maskPosition.cy" :style="{
        ...
        <text x="50%" y="50%" text-anchor="middle" dominant-baseline="middle" stroke="url(#textGradient)"
            :stroke-width="strokeWidth" mask="url(#textMask)"
